...
import SVGColorPalette from './SVGColorPalette';
...
const RightPanel = () => {
    ...
    const isSVGSelected = selectedObject && ['rect', 'circle', 'line', 'path', 'polygon', 'polyline', 'triangle', 'group'].includes(selectedObject?.type)
    const isBrushActive = activeTool === 'brush' || activeTool === 'eraser';

    console.log("isSVGSelected", isSVGSelected, selectedObject?.type)
                    ...
                    <div className="border-b border-gray-200">
                        ...
                        >
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="font-semibold text-gray-900 text-xs font-heading">Color Palette</span>
                            </div>
                            <ChevronDown 
                                size={14} 
                                className={`text-gray-400 transition-transform duration-150 ${openSections.colorPalette ? 'rotate-180' : ''}`}
                            />
                        </button>
                        
                        <div 
                            className={`overflow-hidden transition-all duration-200 ease-in-out ${
                                openSections.colorPalette ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'
                            }`}
                        >
                            <div className="px-2.5 pb-2.5">
                                <SVGColorPalette selectedObject={selectedObject} />
                            </div>
                        </div>
                    </div>
                )}
                
                {isImageOrVectorSelected && !isSVGSelected && <ImagePropertiesPanel embedded={true} flattened={true} />}
            </div>
